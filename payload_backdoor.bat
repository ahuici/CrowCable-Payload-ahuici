...
@REM powershell -WindowStyle Hidden -NoProfile -Command "Add-Type -AssemblyName System.Speech; $speak = New-Object System.Speech.Synthesis.SpeechSynthesizer; $speak.Volume = 100; $speak.Speak('KALISE PARA TODOS. Quien quiere un kalise???');"
...
@REM ipconfig | findstr /i "IPv4" > "%TEMP%\ipline.txt"

@REM set "IP="
@REM for /f "usebackq delims=" %%L in ("%TEMP%\ipline.txt") do (
@REM     set "linea=%%L"
@REM     set "IP=!linea:~-15!"
@REM )
...
@REM set "FILE=%TEMP%\mi_ip.txt"
@REM echo %USERNAME%, se que eres fan de Iniesta. Yo tambien, pero ahora que tengo tu ip %IP% te voy a robar los datos bancarios > "%FILE%"
